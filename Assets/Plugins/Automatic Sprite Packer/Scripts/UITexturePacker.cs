...
public class UITexturePacker
{
    ...
    Rect FindPositionForNewNodeBottomLeft(int width, int height, ref int bestY, ref int bestX)
    {
        ...
        foreach (var rect in freeRectangles)
        {
            ...
            {
                int topSideY = (int)rect.y + height;
    ...
    Rect FindPositionForNewNodeBestShortSideFit(int width, int height, ref int bestShortSideFit, ref int bestLongSideFit)
    {
        ...
        foreach (var rect in freeRectangles)
        {
            ...
            if (rect.width >= width && rect.height >= height)
            {
                int leftoverHoriz = Mathf.Abs((int)rect.width - width);
                int leftoverVert = Mathf.Abs((int)rect.height - height);
            ...
            if (allowRotations && rect.width >= height && rect.height >= width)
            {
                int flippedLeftoverHoriz = Mathf.Abs((int)rect.width - height);
                int flippedLeftoverVert = Mathf.Abs((int)rect.height - width);
    ...
    Rect FindPositionForNewNodeBestLongSideFit(int width, int height, ref int bestShortSideFit, ref int bestLongSideFit)
    {
        ...
        foreach (var rect in freeRectangles)
        {
            ...
            if (allowRotations && rect.width >= height && rect.height >= width)
            {
                int leftoverHoriz = Mathf.Abs((int)rect.width - height);
                int leftoverVert = Mathf.Abs((int)rect.height - width);
                int shortSideFit = Mathf.Min(leftoverHoriz, leftoverVert);
                int longSideFit = Mathf.Max(leftoverHoriz, leftoverVert);
    ...
    Rect FindPositionForNewNodeBestAreaFit(int width, int height, ref int bestAreaFit, ref int bestShortSideFit)
    {
        ...
        foreach (var rect in freeRectangles)
        {
            int areaFit = (int)rect.width * (int)rect.height - width * height;
    ...
    Rect FindPositionForNewNodeContactPoint(int width, int height, ref int bestContactScore)
    {
        ...
        foreach (var rect in freeRectangles)
        {
            ...
            if (rect.width >= width && rect.height >= height)
            {
                int score = ContactPointScoreNode((int)rect.x, (int)rect.y, width, height);
                if (score > bestContactScore)
                {
                    bestNode.x = (int)rect.x;
                    bestNode.y = (int)rect.y;
                    bestNode.width = width;
                    bestNode.height = height;
            ...
            if (allowRotations && rect.width >= height && rect.height >= width)
            {
                int score = ContactPointScoreNode((int)rect.x, (int)rect.y, height, width);
                if (score > bestContactScore)
                {
                    bestNode.x = (int)rect.x;
                    bestNode.y = (int)rect.y;
