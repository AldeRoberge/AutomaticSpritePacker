...
{
    public class ASPReferenceReplacerWindow : EditorWindow
    {
        ...
        {
            GetWindow(typeof(ASPReferenceReplacerWindow));
        }
        ...
        protected void OnEnable()
        {
            ...
            if (EditorPrefs.HasKey(ASPTools.Settings_SavedInstanceIDKey))
            {
                string instancePath = AssetDatabase.GetAssetPath(EditorPrefs.GetInt(ASPTools.Settings_SavedInstanceIDKey, 0));

                if (!string.IsNullOrEmpty(instancePath))
                {
                    m_Instance = AssetDatabase.LoadAssetAtPath(instancePath, typeof(ASPInstance)) as ASPInstance;
                }
            }
        ...
        protected void OnGUI()
        {
            ...
            GUI.changed = false;
            m_Instance = EditorGUILayout.ObjectField("Sprite Packer", m_Instance, typeof(ASPInstance), false) as ASPInstance;
            if (GUI.changed)
            {
                // Save the instance id
                EditorPrefs.SetInt(ASPTools.Settings_SavedInstanceIDKey, (m_Instance == null) ? 0 : m_Instance.GetInstanceID());
            }
            ...
            else
            {
                if (GUILayout.Button("Replace"))
                {
                    ...
                    switch (m_TargetMode)
                    {
                        ...
                        {
                            replacedCount += ASPTools.ReplaceReferencesInScene(m_Instance.copyOfSprites, m_ReplaceMode, spriteRenderersOnly);
                            break;
                        }
                        case TargetMode.ProjectOnly:
                        {
                            replacedCount += ASPTools.ReplaceReferencesInProject(m_Instance.copyOfSprites, m_ReplaceMode, spriteRenderersOnly);
                            break;
                        }
                        ...
                        {
                            replacedCount += ASPTools.ReplaceReferencesInAllScenes(m_Instance.copyOfSprites, m_ReplaceMode, spriteRenderersOnly, false);
                            break;
                        }
                        case TargetMode.AllScenesAndProject:
                        {
                            ...
                            replacedCount += ASPTools.ReplaceReferencesInAllScenes(m_Instance.copyOfSprites, m_ReplaceMode, spriteRenderersOnly, true);
                            break;
                        }
                    }
